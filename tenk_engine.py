...
# ============================================================
# Hard-coded financials (Microsoft + peers for benchmarking)
...
def peer_financials():
    """Simplified peer data for benchmarking. Approximate FY24-FY25 figures."""
    return {
        "Apple": {
            "income": pd.DataFrame([
...
def process_peers(peer_names):
    """Return dict of {company: result} for the requested peers."""
...
def build_scenario_dataframe(
    starting_revenue, start_year=2026, periods=3,
...
def monte_carlo_forecast(
    starting_revenue, mean_growth, std_growth,
    start_year=2026, periods=5, n_simulations=500, seed=42,
):
    """Return a long-form DataFrame of simulated revenue paths + percentile bands."""
...
def sensitivity_grid(
    starting_revenue, starting_op_margin,
    growth_range, margin_range, years_ahead=3,
):
    """Return a 2D grid of projected operating income for growth × margin combos."""
    grid = []
...
# ============================================================
# Display Helpers (kept for backward compat)
# ============================================================
...
# ============================================================
# AI-Style Commentary (with streaming support)
# ============================================================
...
def answer_question(question, df, kpis=None):
    ...
    q = question.lower()
    # Use Microsoft row specifically if present, otherwise first row
    ...
    if "forecast" in q or "projection" in q or "future" in q:
        base_forecast = build_forecast_dataframe(
            starting_revenue=revenue, growth_rate=0.12,
            start_year=2026, periods=3,
        )
        final_year = int(base_forecast.iloc[-1]["year"])
        final_revenue = base_forecast.iloc[-1]["revenue"]
        return (
            f"Using a 12.0% annual revenue growth assumption, Microsoft revenue would reach "
            f"{fmt_cur(final_revenue)} by FY{final_year}. This is a simplified scenario, not a formal forecast."
        )
    ...
    if "peer" in q or "compare" in q or "benchmark" in q:
        if len(df) > 1:
            ...
            for _, row in df.iterrows():
                lines.append(
                    f"{row['company']}: revenue {fmt_cur(row['revenue'])}, "
                    f"op margin {fmt_pct(row['operating_margin'])}, "
                    f"FCF {fmt_cur(row['free_cash_flow'])}."
                )
    ...
    if "revenue" in q or "growth" in q:
        ...
        if service_mix is not None:
            mix_text = (
                ...
                f"making it the largest category."
            )
        return (
    ...
    if "margin" in q or "profit" in q:
        return (
            f"Microsoft shows a strong profitability profile: gross margin {fmt_pct(gross_margin)}, "
            f"operating margin {fmt_pct(operating_margin)}, and net margin {fmt_pct(net_margin)}."
        )

    if "cash" in q or "fcf" in q or "liquidity" in q:
        debt_to_cash = debt / cash if cash else None
        return (
            f"Microsoft generated {fmt_cur(fcf)} in free cash flow with {fmt_cur(cash)} in cash. "
            f"Total debt was {fmt_cur(debt)} — debt-to-cash ratio of ~{debt_to_cash:.1f}x."
        )

    if "debt" in q or "balance sheet" in q:
        debt_to_cash = debt / cash if cash else None
        return (
            f"Balance sheet: {fmt_cur(cash)} of cash vs {fmt_cur(debt)} of total debt "
            f"(debt-to-cash ~{debt_to_cash:.1f}x)."
        )

    return (
        f"Microsoft generated {fmt_cur(revenue)} of FY{r['year']} revenue, grew {fmt_pct(yoy)} YoY, "
        f"operating margin {fmt_pct(operating_margin)}, and produced {fmt_cur(fcf)} in free cash flow."
    )


def stream_answer(question, df, kpis=None):
    """Generator that yields the answer word-by-word for st.write_stream."""
    import time
