...
def extract_segment_revenue(tables: List[pd.DataFrame]) -> pd.DataFrame:
    segment_keywords = [
        "segment", "productivity", "cloud", "services", "devices",
        "advertising", "geographic", "business unit",
    ]
    rows = []
    for raw_table in tables:
    ...
    if any(term in q for term in ["margin", "profitability", "gross", "operating"]):
        ...
        for _, row in scoped.iterrows():
            lines.append(
                ...
                f"revenue YoY {format_percent(row.get('revenue_yoy_growth'))}, "
                f"cash {format_currency(row.get('cash_balance'))}, "
                f"debt {format_currency(row.get('total_debt'))}."
            )
    return "\n".join(lines) if lines else "No specific insight could be generated for that question."
...
st.write("Upload one or more 10-K filings (PDF, HTML, or TXT) to extract KPIs, compare competitors, and visualize trends.")
...
question = st.text_input(
    "Ask a financial question about these companies (e.g., 'Who has the best margins?' or 'Which company has the strongest cash flow?')",
    "",
)
...
if uploaded_files:
    for file in uploaded_files:
        with st.spinner(f"Processing {file.name}..."):
            ...
            company_results[company_name] = {
                ...
                "segment_revenue": extract_segment_revenue(tables),
            }
    ...
    if not benchmark_df.empty:
        ...
        if seg_frames:
            ...
            st.info("No segment revenue tables were detected in the uploaded filings.")
